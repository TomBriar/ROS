...
# This file was *autogenerated* from the file Schnorr3.sage
...
def hash(byteslist):
	return IntPrime(int(hashlib.sha256(byteslist).hexdigest(), _sage_const_16 ))
...
def kListHROS(w, L, R):
	...
	for x in range(_sage_const_0 , w): #add one because SUM is inclusive and end range is exclusive minus one because the range should be 1 - w
		...
		for j in range(_sage_const_0 , _sage_const_2 **(level-_sage_const_1 )): #add one because SUM is inclusive and end range is exclusive minus one because we are indexing from 0
			print("j "+str(j))
			FLi.append(join(TreeLevel[j*_sage_const_2 -_sage_const_1 ],TreeLevel[_sage_const_2 *j],Ii(level))) #minus one because lists are 1 indexed in the paper
		Tree.append(FLi)
