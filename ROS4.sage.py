...
# This file was *autogenerated* from the file ROS4.sage
...
def hashX(x, aux = 'aux321'):
	return IntPrime(Ring(int(hashlib.sha256(bytes(str(aux), "ascii")+bytes(str(x), "ascii")).hexdigest(),_sage_const_16 )))
...
def genCaux():
	...
	for i in range(_sage_const_0 , l):
		caux["p"+str(i)] = [hashX(encode(X[i]), _sage_const_0 ), hashX(encode(X[i]), _sage_const_1 )]
	return caux
...
for i in range(_sage_const_0 , l):
		r = int(K[i]) * G
		R.append(r)
		c0 = hashR(encodeR(r), M[i][_sage_const_0 ])
		c1 = hashR(encodeR(r), M[i][_sage_const_1 ])
		CB.append([c0, c1])

# Generate xi' 
xP = []
for i in range(_sage_const_0 ,l):
	a = X[i]-CB[i][_sage_const_0 ]
...
def PlR(x):
	...
	for i in range(_sage_const_0 , len(Pli)-_sage_const_1 ):
